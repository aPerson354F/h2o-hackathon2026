// Fills in any languages missing from translations.json using Google Translate's
// public unofficial endpoint (no key, no rate limits at this volume).
// Run: node i18n/fill-missing.mjs
...
// Map our codes → Google Translate's codes where they differ
const GOOGLE_CODE = {
  fil: "tl", // Tagalog
  zh: "zh-CN",
  fa: "fa",
  he: "iw", // Google's older Hebrew code (still accepted)
  no: "no",
  my: "my",
};
...
async function translateOne(text, target) {
  ...
  // Preserve {placeholder} tokens by wrapping with sentinels Google won't translate.
  const placeholders = [];
...
const missing = LANGS.filter(
  (l) => !existing[l.code] || Object.keys(existing[l.code]).length < expected,
);
console.log(`${missing.length} missing languages, ${LANGS.length - missing.length} already done.`);
...
for (const lang of missing) {
  ...
  try {
    const translations = await pool(keys, 8, async (k) => {
      try {
        return await translateOne(STRINGS[k], lang.code);
      } catch {
        return STRINGS[k]; // English fallback per-string on error
      }
    });
    const dict = Object.fromEntries(keys.map((k, i) => [k, translations[i]]));
    existing[lang.code] = dict;
    writeFileSync(TRANS_PATH, JSON.stringify(existing, null, 2));
    console.log(`✓ ${Object.keys(dict).length} keys (${Date.now() - t0}ms)`);
  } catch (e) {
    console.log(`✗ ${e.message}`);
  }
}

const total = LANGS.filter((l) => existing[l.code]).length + 1; // +en
console.log(`\nDone. ${total}/${LANGS.length + 1} languages in translations.json.`);
